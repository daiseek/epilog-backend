...
'''SSE 스트리밍 연결을 위한 뷰 함수 - 클라이언트에게 캐릭터/대본 작업 이벤트 메시지 전송용'''
def character_task_eventstream_view(request, task_id):
    """
    직접 구현한 SSE 스트리밍 뷰 (캐릭터/대본 작업용)
    """
    def event_generator():
        import redis
        import json
        ...
        print(f"[SSE] Characters 클라이언트 연결됨 - 채널: {channel}")
        
        try:
            # 연결 성공 메시지
            yield f"event: connected\n"
            yield f"data: {json.dumps({'message': 'Characters SSE 연결 성공', 'channel': channel})}\n\n"
            ...
            for message in pubsub.listen():
                if message['type'] == 'message':
                    try:
                        ...
                        print(f"[SSE] Characters 메시지 파싱 오류: {e}")
                        
        except Exception as e:
            print(f"[SSE] Characters 연결 오류: {e}")
        finally:
            pubsub.close()
            print(f"[SSE] Characters 연결 종료 - 채널: {channel}")
